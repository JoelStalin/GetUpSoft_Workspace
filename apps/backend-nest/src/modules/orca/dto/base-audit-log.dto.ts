...
/**
 * Base DTO for ORCA audit logs - covers all Odoo models
 * Extends AuditLogRequestDto with additional fields for project tracking and sync status
 */
export class BaseAuditLogDto {
  @ApiProperty({ description: 'Project identifier for multi-tenant tracking' })
  @IsString()
  project_id!: string;
  ...
  @ApiProperty({ description: 'Whether this log has been synced to ORCA', default: false, required: false })
  @IsBoolean()
  @IsOptional()
  orca_synced?: boolean;
  ...
  @ApiProperty({ description: 'Request ID returned by ORCA on successful push', required: false })
  @IsString()
  @IsOptional()
