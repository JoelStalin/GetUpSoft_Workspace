...
web_search
  query: string (required) â€” write a clear, focused search query
...
file_controller
  ...
  path: string â€” use "desktop" for Desktop folder
  name: string â€” filename
  content: string â€” file content (for write/create_file)

computer_settings
  action: string (required)
  description: string â€” natural language description
...
screen_process
  text: string (required) â€” what to analyze or ask about the screen
...
OUTPUT â€” return ONLY valid JSON, no markdown, no explanation, no code blocks:
{
  "goal": "...",
  "steps": [
    {
    ...
    try:
        ...
        for step in plan["steps"]:
            if step.get("tool") in ("generated_code",):
                print(f"[Planner] âš ï¸ generated_code detected in step {step.get('step')} â€” replacing with web_search")
                desc = step.get("description", goal)
                step["tool"] = "web_search"
                step["parameters"] = {"query": desc[:200]}

        print(f"[Planner] âœ… Plan: {len(plan['steps'])} steps")
        for s in plan["steps"]:
        ...
        print(f"[Planner] âš ï¸ JSON parse failed: {e}")
        return _fallback_plan(goal)
    except Exception as e:
        print(f"[Planner] âš ï¸ Planning failed: {e}")
        return _fallback_plan(goal)


def _fallback_plan(goal: str) -> dict:
    print("[Planner] ðŸ”„ Fallback plan")
    return {
        "goal": goal,
        "steps": [
            {
...
Create a REVISED plan for the remaining work only. Do not repeat completed steps."""
    ...
    try:
        ...
        print(f"[Planner] ðŸ”„ Revised plan: {len(plan['steps'])} steps")
        return plan
    except Exception as e:
        print(f"[Planner] âš ï¸ Replan failed: {e}")
        return _fallback_plan(goal)
