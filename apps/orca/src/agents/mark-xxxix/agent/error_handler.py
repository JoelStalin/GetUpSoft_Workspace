...
def analyze_error(
    ...
    if attempt >= max_attempts:
        print(f"[ErrorHandler] âš ï¸ Max attempts reached for step {step.get('step')} â€” forcing replan")
        return {
...
Attempt number: {attempt}"""
    ...
    try:
        ...
        if step.get("critical") and result["decision"] == ErrorDecision.SKIP:
            ...
            result["user_message"] = "This step is critical â€” finding alternative approach, sir."

        print(f"[ErrorHandler] Decision: {result['decision'].value} â€” {result.get('reason', '')}")
        return result

    except Exception as e:
        print(f"[ErrorHandler] âš ï¸ Analysis failed: {e} â€” defaulting to replan")
        return {
            "decision":       ErrorDecision.REPLAN,
            "reason":         str(e),
...
Return ONLY the Python code, no explanation."""
    ...
    try:
        ...
        print(f"[ErrorHandler] âš ï¸ Fix generation failed: {e}")
        return {
            "step":        step.get("step"),
            "tool":        "generated_code",
