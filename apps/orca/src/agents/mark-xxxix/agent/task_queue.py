...
class Task:
    ...
    project_id:  str   = field(compare=False, default="unknown") # Added project identifier
...
class TaskQueue:
    ...
    def submit(
        self,
        goal:        str,
        project_id:  str = "orca-internal", # New parameter
        ...
        task    = Task(
            ...
            task_id     = task_id,
            goal        = goal,
            project_id  = project_id, # Assign project_id
            speak       = speak,
        ...
        print(f"[TaskQueue] 📥 Task queued: [{task_id}] (Project: {project_id}) {goal[:60]}")
        return task_id
    ...
    def _run_task(self, task: Task) -> None:
        print(f"[TaskQueue] ▶️ Running: [{task.task_id}] (Project: {task.project_id}) {task.goal[:60]}")
        try:
            ...
            if task.on_complete and not task.cancel_flag.is_set():
                try:
                    ...
                    print(f"[TaskQueue] âš ï¸ on_complete callback error: {e}")

            print(f"[TaskQueue] âœ… Completed: [{task.task_id}]")
            ...
            with self._lock:
                ...
                task.error  = str(e)
                self._active_count -= 1
            print(f"[TaskQueue] âŒ Failed: [{task.task_id}] {e}")
