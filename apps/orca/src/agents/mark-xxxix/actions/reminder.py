...
"""
    ...
    script_body = f"""# Auto-generated by J.A.R.V.I.S reminder â€” do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    if result.returncode != 0:
        ...
        print(f"[Reminder] âŒ schtasks: {err}")
        return ""  
...
def _schedule_mac(target_dt: datetime, task_name: str,
    ...
    label     = f"com.Orca.reminder.{task_name}"
...
"""
    ...
    if result.returncode != 0:
        ...
        script_path.unlink(missing_ok=True)
        print(f"[Reminder] âŒ launchctl: {result.stderr.strip()}")
        return ""
...
def _schedule_linux(target_dt: datetime, task_name: str,
    ...
    if shutil.which("systemd-run"):
        ...
        if result.returncode == 0:
            return task_name
        print(f"[Reminder] âš ï¸ systemd-run failed: {result.stderr.strip()}, trying 'at'")
    ...
    if shutil.which("at"):
        ...
        if result.returncode == 0:
            return task_name
        print(f"[Reminder] âŒ at: {result.stderr.strip()}")
        return ""

    print("[Reminder] âŒ Neither systemd-run nor at found on this Linux system.")
    return ""
...
def reminder(
    ...
    if target_dt <= datetime.now():
        return "That time has already passed â€” I can't set a reminder in the past."

    os_name    = _get_os()
    safe_msg   = _sanitise(message)
    task_name  = f"OrcaReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        script_path.unlink(missing_ok=True)
        print(f"[Reminder] âŒ Scheduling exception: {e}")
    ...
    if player:
        player.write_log(f"[Reminder] âœ… {date_str} {time_str} â€” {safe_msg[:40]}")
