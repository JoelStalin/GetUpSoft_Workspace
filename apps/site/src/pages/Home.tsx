...
export function HomePage() {
  ...
  const methRef = useScrollReveal<HTMLDivElement>({ childSelector: ".meth-step" });

  return (
    <div className="bg-background">
      {/* Hero Section */}
      <Section className="relative overflow-hidden !py-32 lg:!py-48">
        {/* Aesthetic backgrounds with animation */}
        ...
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-accentPurpleSoft/20 rounded-full blur-[100px] -ml-24 -mb-24 animate-float" />
        ...
        <Container className="relative">
          <div className="max-w-4xl animate-fade-in-slow">
            ...
            <div className="flex flex-col sm:flex-row gap-5">
              <Button to="/contact" className="px-10 py-5 shadow-soft-xl hover:scale-105 transition-transform">
          ...
          {/* Trust Bar */}
          <div className="mt-24 pt-10 border-t border-border animate-fade-in-slow" style={{ animationDelay: '400ms' }}>
      ...
      <Section background="surface">
        <Container>
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            <div ref={capRef} className="lg:col-span-2 grid sm:grid-cols-2 gap-6">
              {capabilities.map((cap) => (
                <div key={cap.title} className="cap-card p-8 rounded-3xl bg-background border border-border shadow-soft-xl hover:shadow-soft-2xl hover:-translate-y-1 transition-all duration-300 group">
                  <div className={`text-4xl mb-6 text-primary transition-transform group-hover:scale-110`}>{cap.icon}</div>
                  <h3 className="text-xl font-bold text-text mb-4">{cap.title}</h3>
                  <p className="text-textMuted leading-relaxed">{cap.description}</p>
                </div>
              ))}
              <div className="cap-card p-8 rounded-3xl bg-primary flex flex-col justify-between text-white shadow-soft-xl group hover:shadow-soft-2xl transition-all">
                <div>
                ...
                <Button variant="ghost" to="/integrations" className="!p-0 !text-white hover:!bg-transparent mt-8 self-start group">
                  View Connectors
                </Button>
              </div>
            </div>
      ...
      <Section>
        <Container>
          ...
          <div ref={methRef} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {methodology.map((step) => (
              <div key={step.step} className="meth-step relative p-8 group">
      ...
      <Section background="primarySoft" className="!py-24">
        <Container>
          <div className="bg-background p-12 sm:p-20 rounded-[48px] shadow-soft-2xl flex flex-col lg:flex-row items-center justify-between gap-12 text-center lg:text-left overflow-hidden relative group">
            <div className="absolute top-0 right-0 w-64 h-64 bg-accentTealSoft/30 rounded-full blur-[80px] -mr-32 -mt-32 transition-transform duration-700 group-hover:scale-125" />
            ...
            <div className="relative z-10 flex flex-col sm:flex-row gap-4">
              <Button to="/contact" className="px-12 py-5 shadow-soft-xl hover:scale-105 transition-transform">Get Started</Button>
