...
class CustomPortal(CustomerPortal):
    ...
    @http.route(['/my/invoices', '/my/invoices/page/<int:page>'], type='http', auth="user", cors="*")
    ...
    def get_invoices(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        for inv in invoices:
            invoice_data = {
                ...
                'document_fiscal_position_code_prefix': inv.l10n_latam_document_type_id.doc_code_prefix if inv.l10n_latam_document_type_id else False,
                'state': inv.state,
                ...
                'subtotal_internal_loads': inv.subtotal_internal_loads,
                'subtotal_internal_loads_discount': inv.subtotal_internal_loads_discount,
                'subtotal_internal_loads_additional': inv.subtotal_internal_loads_additional,
                'subtotal_internal_discounts': inv.subtotal_internal_discounts,
                'subtotal_internal_loads_subtotal': inv.subtotal_internal_loads_subtotal,
            }
    ...
    def _get_portal_invoices_data(self, page, date_begin, date_end, sortby, **kw):
        ...
        records_per_page = int(limit) if str(limit).isdigit() else 50
    ...
    def get_api_invoice(self, move_id):
        try:
            ...
            if not move.exists():
                return http_handler_error(f"Invoice with ID {move_id} not found", 500, '500 Internal Server Error')
            result = {'invoice': move.sudo().get_info()}
            return http_response_success(result)
...
class AccountController(http.Controller):
    ...
    def get_load_status(self, load_number=None):
        query = [('load_number', '=', load_number)] if load_number else [('load_number', '!=', False)]
        move_lines = request.env['account.move.line'].sudo().search(query)
        loads = {}
        for move_line in move_lines:
            load_num = move_line['load_number']
            move_type = move_line['move_id']['move_type']
            invoice_name = self.get_invoice_name_by_move_type(move_type)
            status = self.get_invoice_status(move_line['move_id'])
            if loads.get(load_num):
                loads[load_num][invoice_name] = status
            else:
                loads[load_num] = {invoice_name: status}
        data = [{"load_number": k, "status": v} for k, v in loads.items()]
        return http_response_success(data)
    ...
    def get_controller_loads(self):
        ...
        for line in account_move_lines:
            load_num = line['load_number']
            move_type = line['move_id']['move_type']
            parent_state = line['parent_state']
            if load_numbers.get(load_num):
                if move_type == 'out_invoice':
                    load_numbers[load_num]['invoice_status'] = parent_state
                if move_type == 'in_invoice':
                    load_numbers[load_num]['bill_status'] = parent_state
            else:
                load_numbers[load_num] = {
                    'load_number': load_num,
                    'invoice_status': parent_state if move_type == 'out_invoice' else False,
                    'bill_status': parent_state if move_type == 'in_invoice' else False
                }
        loads = list(load_numbers.values())
    ...
    def get_tables(self, table, key=None, compare=None, value=None):
        if key and compare and value:
            records = request.env[table].with_context(bin_size=True).sudo().search([(key, compare, value)])
        else:
            records = request.env[table].with_context(bin_size=True).sudo().search([])
        data = Serializer(records, "{*}", many=True).data
        return http_response_success(data)
    ...
    @http.route('/api/account/move/<int:move_id>/confirmate/', type='json', methods=['POST'], auth='public', website=True)
    def confirm_move_state_post(self, move_id, state, reason=None, doc_number=None, transporter_name=None):
        move = request.env['account.move'].sudo().browse(move_id)
        move.with_user(SUPERUSER_ID).message_post(
            body=f"Transportista: {transporter_name or 'Sin Transportista'}\n Estado de la factura cambiado a: {state}. \n Razón: {reason or 'No especificada'}. \n y doc number {doc_number}",
            subtype_xmlid='mail.mt_note', message_type='notification'
        )
        return response_success({"Ok": True})

    def _confirm_move_state(self, move_id, state, reason=None, doc_number=None, transporter_name=None):
        move = request.env['account.move'].sudo().browse(move_id)
        init_move_update = {
    ...
    def test_api_key(self, id, mail_create_nosubscribe, mail_post_autofollow, message, message_type, subtype_xmlid):
        ...
        ).message_post(body=message, subtype_xmlid=subtype_xmlid, message_type=message_type)
