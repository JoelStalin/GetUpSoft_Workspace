...
#       `${config.get("odooPath")}customer_invoice`,
#       {
#         partner: {
#           name: customer.name,
#           rnc: customer.RNC_no,
...
#             code: product.qrCode || product.defaultQRCode,
#             quantity: 1,
#             price: booking.price,
#             load: {
#               loadId: booking.loadMap,
#               loadNumber: loadMap.loadNumber,
#             },
#           },
#         ],
#       },
...
#     const { status, invoice_id, invoice_number, pdf_url } = response.data;
# ```

    @http.route(['/api/invoice/prepayment'], type='json', auth="public", methods=['POST'],  csrf=False) # Tested
    def create_draft_with_payment(self, **info):
        rnc = info.get('rnc', False)
        prepayment_id = info.get('prepaymentId', False)
        load_id = info.get('load_id', False) 
        ncf = info.get('ncf', False) 
        concept = info.get('concept', False)
        amount_of_payment = info.get('amountOfPayment', False)


        # La Carga y el monto que paga...
        # El sistema va usar el metodo que tenemos hoy en dia automatic y nosotros le pasaramos un id de carga para que sea el que haga la magia. Luego buscaremos una borrador que tenga esa carga y le aplicamos el pago.


        load_ids = [load_id]
        if (not load_id or len(load_id) == 0):
            return http_handler_error("No se ha recibido la carga", 400, '400 Bad Request')
        ...
        try:
            ...
            line_load = http.request.env['account.line.load'].sudo().search([('move_type', '=', 'invoice'), '|', ('load_id', '=', load_id), ('original_load_id', '=', load_id)], limit = 1)
            if not line_load or not line_load.account_move_id:
                return http_handler_error("No se pudo procesar el pago para esta carga. Razon: la carga no pudo ser registrada en la factura. Favor contactar al equipo de Contabilidad de FLAI para ajustar el pago", 400, '400 Bad Request')
            

            if ncf and not line_load.account_move_id.l10n_latam_document_number:
                line_load.account_move_id.sudo().write({
                    'l10n_latam_document_number': ncf
                })
            
        
            # TODO: Este es el codigo que se debe crear.
            line_load.account_move_id.register_payment(amount_of_payment)
            moves = [line_load.account_move_id]
