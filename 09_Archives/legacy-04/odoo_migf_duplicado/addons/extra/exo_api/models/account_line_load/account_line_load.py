...
class AccountLineLoad(models.Model):
    ...
    move_type = fields.Selection([
        ('invoice', 'Factura de un Shipper'),
        ('bill', 'Factura de un Transportista')
    ], help="Indica si la carga se creo a partir de un factura de un cliente o de una factura de un tranportista")
    ...
    def create_in_invoice(self, current_env=None):
        ...
        for line in self.filtered(lambda l: not l.account_line_id):
            ...
            additional_taxes = current_transporter_partner.tax_products.filtered(
                lambda x: x.product_tmpl_id.id == product['id']
            ) if current_transporter_partner.tax_products else []
            ...
            if not exists_invoice_from_partner:
                ...
                if not clients.get(client_key_group):
                    clients[client_key_group] = {
                        "partner": current_transporter_partner,
                        "block": {
                            'block_start': line.block_date_start,
                            'block_end': line.block_date_end
                        },
                        'load_invoice_code': group_key,
                        "invoice_line_ids": []
                    }
        ...
        for record_chunk in self.chunk_records(50):
            ...
            if exo_json_data and len(exo_json_data) > 0:
                for record in record_chunk:
                    json_value = next((item for item in exo_json_data if item["loadNumber"].lower() == record.load_number.lower()), None)
                    if json_value:
                        record.sudo().write({
    ...
    def unlink(self):
        ...
        return super(AccountLineLoad, self).unlink()
    ...
    def getLoadStatuses(self, language='en'):
        ...
        for load_id, records in groups.items():
            ...
            created_shipper_invoice_date = self.get_min_date([
                datetime.combine(line_load['account_move_id']['invoice_date'], datetime.min.time())
                for line_load in records if line_load['move_type'] == 'invoice' and line_load["account_move_id"].state in ['posted']
            ])
            created_transporter_invoice_date = self.get_min_date([
                datetime.combine(line_load['account_move_id']['invoice_date'], datetime.min.time())
                for line_load in records if line_load['move_type'] == 'bill' and line_load["account_move_id"].state in ['posted']
            ])
            shipper_payment_state_invoice_date_list_dates = [
                line_load['create_date']
                for line_load in records if line_load['move_type'] == 'invoice' and line_load["account_move_id"].payment_state in ['paid', 'reversed'] and line_load['account_move_id']['last_payment']
            ]
            shipper_payment_state_invoice_date = datetime.combine(
                min(shipper_payment_state_invoice_date_list_dates), datetime.min.time()
            ).timestamp() if shipper_payment_state_invoice_date_list_dates else None
            transporter_payment_state_invoice_list_dates = [
                line_load['create_date']
                for line_load in records if line_load['move_type'] == 'bill' and line_load["account_move_id"] and line_load["account_move_id"].payment_state in ['paid', 'reversed'] and line_load['account_move_id']['last_payment']
            ]
            transporter_payment_state_invoice_date = datetime.combine(
                min(transporter_payment_state_invoice_list_dates), datetime.min.time()
            ).timestamp() if transporter_payment_state_invoice_list_dates else None
            shipper_payment_state_invoice_comments_list = [
                PAYMENT_STATE_SELECTION[language][line_load["account_move_id"].payment_state]
                for line_load in records if line_load["account_move_id"] and line_load['move_type'] == 'invoice'
            ]
            shipper_payment_state_invoice_comment = (
                f"{MESSAGE_LANGUAGE_SELECTION[language]['invoice_of_shipper']}: {shipper_payment_state_invoice_comments_list[0]}"
                if len(shipper_payment_state_invoice_comments_list) > 0
                else MESSAGE_LANGUAGE_SELECTION[language]['without_payment_status']
            )
            transporter_payment_state_bill_comments_list = [
                PAYMENT_STATE_SELECTION[language][line_load["account_move_id"].payment_state]
                for line_load in records if line_load["account_move_id"] and line_load['move_type'] == 'bill'
            ]
            transporter_payment_state_bill_comment = (
                f"{MESSAGE_LANGUAGE_SELECTION[language]['invoice_of_shipper']}: {transporter_payment_state_bill_comments_list[0]}"
                if len(transporter_payment_state_bill_comments_list) > 0
                else MESSAGE_LANGUAGE_SELECTION[language]['without_payment_status']
            )
            in_shipper_draft_invoice = self.get_min_date([
                line_load['account_move_id']['create_date']
                for line_load in records if line_load['move_type'] == 'invoice'
            ])
            in_transporter_draft_invoice = self.get_min_date([
                line_load['account_move_id']['create_date']
                for line_load in records if line_load['move_type'] == 'bill'
            ])
            load_status_obj = {
                "rcv_accounting": {
                    'name': 'rcv_accounting',
                    'date': rcv_accounting_date,
                    'description': MESSAGE_LANGUAGE_SELECTION[language]['received_in_accounting'],
                    'completed': False
                },
                "in_shipper_draft_invoice": {
                    'name': 'in_shipper_draft_invoice',
                    'date': in_shipper_draft_invoice,
                    'description': MESSAGE_LANGUAGE_SELECTION[language]['in_shipper_draft_invoice'],
                    'completed': False
                },
                "in_transporter_draft_invoice": {
                    'name': 'in_transporter_draft_invoice',
                    'date': in_transporter_draft_invoice,
                    'description': MESSAGE_LANGUAGE_SELECTION[language]['in_transporter_draft_invoice'],
                    'completed': False
                },
                "in_shipper_approved_invoice": {
                    'name': 'in_shipper_approved_invoice',
                    'date': created_shipper_invoice_date,
                    'description': MESSAGE_LANGUAGE_SELECTION[language]['in_shipper_approved_invoice'],
                    'completed': False
                },
                "in_transporter_approved_invoice": {
                    'name': 'in_transporter_approved_invoice',
                    'date': created_transporter_invoice_date,
                    'description': MESSAGE_LANGUAGE_SELECTION[language]['in_transporter_approved_invoice'],
                    'completed': False
                },
                "created_shipper_invoice": {
                    'name': 'created_shipper_invoice',
                    'date': created_shipper_invoice_date,
                    'description': MESSAGE_LANGUAGE_SELECTION[language]['invoiced_shipper'],
                    'completed': False
                },
                "created_transporter_invoice": {
                    'name': 'created_transporter_invoice',
                    'date': created_transporter_invoice_date,
                    'description': MESSAGE_LANGUAGE_SELECTION[language]['billed_transporter'],
                    'completed': False
                },
                "shipper_payment_state_invoice": {
                    'name': 'shipper_payment_state_invoice',
                    'date': shipper_payment_state_invoice_date,
                    'description': f"{MESSAGE_LANGUAGE_SELECTION[language]['invoice_of_shipper']}: ({shipper_payment_state_invoice_comment})",
                    'completed': False
                },
                "transporter_payment_state_invoice": {
                    'name': 'transporter_payment_state_invoice',
                    'date': transporter_payment_state_invoice_date,
                    'description': f"{MESSAGE_LANGUAGE_SELECTION[language]['bill_transporter']}: ({transporter_payment_state_bill_comment})",
                    'completed': False
                },
                "paid_by_shipper": {
                    'name': 'paid_by_shipper',
                    'date': shipper_payment_state_invoice_date,
                    'description': MESSAGE_LANGUAGE_SELECTION[language]['paid_by_shipper'],
                    'completed': False
                },
                "paid_to_transporter": {
                    'name': 'paid_to_transporter',
                    'date': transporter_payment_state_invoice_date,
                    'description': MESSAGE_LANGUAGE_SELECTION[language]['paid_to_transporter'],
                    'completed': False
                },
            }
            for key in load_status_obj:
                load_status_obj[key]['completed'] = load_status_obj[key]['date'] is not None
            statuses = [load_status_obj[load_status] for load_status in load_status_obj]
