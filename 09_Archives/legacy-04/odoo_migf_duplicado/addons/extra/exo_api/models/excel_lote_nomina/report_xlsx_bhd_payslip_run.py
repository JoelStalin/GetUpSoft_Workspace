...
class ReportPayslipRunBHDXlsx(models.AbstractModel):
    _name = 'report.exo_api.payslip_run_bhd_xlsx'
    ...
    HEADERS = [
        ...
        'No. de cuenta', 'Correo beneficiario', 'Fax beneficiario', 'Referencia débito',
    ]
    ...
    def _build_bhd_bank_cache(self, all_lines):
    ...
    def generate_xlsx_report(self, workbook, data, runs):
        ...
        partner_bank_map, partners_missing = self._build_bhd_bank_cache(all_lines)
        if partners_missing:
        ...
        for run in runs:
            ...
            for slip, slip_lines in lines_by_slip.items():
                ...
                sheet.write(row, 0, '1')
                sheet.write(row, 1, acc_holder)
                sheet.write_number(row, 2, amount)
                sheet.write(row, 3, '')
