...
_logger_benefits = logging.getLogger("benefits")

handler = logging.FileHandler('/var/log/odoo/benefits.log')
formatter = logging.Formatter('%(asctime)s %(levelname)s: %(message)s')
handler.setFormatter(formatter)

if not _logger_benefits.handlers:  
    _logger_benefits.addHandler(handler)
    _logger_benefits.setLevel(logging.DEBUG)



class PartnerBenefitDiscount(models.Model):
    _name = "partner.benefit.discount"
    _description = "Configuración de Beneficio o Descuento del Proveedor"
    ...
    partner_id = fields.Many2one(
        ...
        string="Proveedor",
        required=True,
        domain=lambda self: [('internal_partner_type', '=', 'transporter')]
    )
    ...
    benefit_discount_id = fields.Many2one(
        'benefit.discount',
        string="Beneficio y/o descuento",
        required=True
    )
    ...
    field_test = fields.Char("Campo de Prueba")
    ...
    # 🔑 clave para manejar acumulación mensual
    month_start = fields.Date(
        string="Inicio de Mes",
        required=True,
    ...
    _sql_constraints = [
        (
            'unique_partner_benefit_month',
            'unique(partner_id, benefit_discount_id, month_start)',
            'Ya existe un beneficio configurado para este partner, beneficio y mes.'
        )
    ]
...
class partner_inherit(models.Model):
        ...
        if 'vat' in vals and vals['vat']:
            ...
            if vals.get('internal_partner_type', None) == 'transporter':
                ...
                if len(to_insert_exo_transporters) == 0:
                    raise ValidationError(f"1. No se encontraron transportistas con el número de identificación (RNC) {vals['vat']}")
    ...
    def write(self, vals):
        ...
        if vals.get('vat', None) or vals.get('internal_partner_type', None) == 'transporter':
            ...
            if internal_partner_type == 'transporter':
                ...
                if internal_partner_type and len(to_insert_exo_transporters) == 0 and not self.partner_object_ids:
                    raise ValidationError(f"2. No se encontraron transportistas con el número de identificación (RNC) {vals['vat']}")
                vals['partner_object_ids'] = to_insert_exo_transporters
    ...
    def set_exo_transporters(self):
        ...
        if len(to_insert) == 0 and not has_data:
            raise ValidationError(f"3. No se encontraron transportistas con el número de identificación (RNC) {self.vat}")
    ...
    @api.model
    def _cron_generate_pending_benefits(self):
        import json
        from ...helpers.request_helper import get_request_exo
    
        log_file_path = '/var/log/odoo/benefits.log'
        with open(log_file_path, "w"):
            pass
        _logger_benefits.info("CRON: Iniciando la generación de beneficios pendientes...")
        ...
        for partner in partners:
            try:
                pending_benefit_model = self.env['partner.benefit.pending'].sudo()
                _logger_benefits.info(">>>> Procesando partner [%s] %s", partner.id, partner.name)
    
                total_vehicles = 0
                vehicles_data = []
    
                for obj in partner.partner_object_ids:
                    try:
                        response = get_request_exo(f"exo/transporter/vehicles/{obj.name}")
                        vehicles = response.get("Result", {}).get("vehicles", [])
                        total_vehicles += len(vehicles)
                        vehicles_data.extend(vehicles)
                        _logger_benefits.info("    >> Obtenidos %s vehículos para object_id: %s", len(vehicles), obj.name)
                    except Exception as e:
                        _logger_benefits.warning("    ✗ No se pudo obtener vehículos para %s: %s", obj.name, e)
    
                if total_vehicles == 0:
                    _logger_benefits.info("    [SKIPPED] No se encontraron vehículos para %s", partner.name)
                    continue
                
                for p_benefit in partner.benefit_discount_ids:
                    for cicle in p_benefit.benefit_discount_id.benefit_discount_cicles_ids:
                        product = p_benefit.benefit_discount_id.product_tmpl_id
                        try:
                            pending_benefit_model.create({
                                'partner_id': partner.id,
                                'partner_benefit_discount_id': p_benefit.id,
                                'benefit_discount_cicle_id': cicle.id,
                                'quantity': total_vehicles,
                                'amount': product.list_price,
                                'generation_date': start_of_month,
                                'vehicles_json': json.dumps(vehicles_data, ensure_ascii=False),
                            })
                            _logger_benefits.info("        ✓ Creado pendiente para %s (%s)", partner.name, start_of_month)
                        except Exception as e:
                            _logger_benefits.error("        ✗ Error creando pendiente: %s", e)
                            self.env.cr.rollback()
                            continue
                self.env.cr.commit()
            except Exception as e:
                _logger_benefits.error("CRON: Error generando beneficios para %s: %s", partner.name, e)
                self.env.cr.rollback()
    
        _logger_benefits.info("CRON: Finalizada la generación de beneficios pendientes.")
        return True
        ...
        for p_benefit_discount in self.benefit_discount_ids:
            for cicle in p_benefit_discount.benefit_discount_id.benefit_discount_cicles_ids:
                if cicle.next_curt_date.day <= current_date.day:
                    benefits_line_to_create.append({
                        'partner_id': self.id,
                        'p_benefit_discount': p_benefit_discount,
                        'cicle': cicle,
                    })
