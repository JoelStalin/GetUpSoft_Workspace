...
class ExoOdooConciliation(models.Model):
    ...
    companyTransporter = fields.Text(string='Transportista')
    createdAt = fields.Date(string='Created At')
    load_status = fields.Text(string='Estado carga en EXO')
    shipper_account_line_load_id = fields.Many2one(
        'account.line.load', 'Shipper Odoo - EXO LOAD', ondelete='cascade')
    provider_account_line_load_id = fields.Many2one(
        'account.line.load', 'Provider Odoo - EXO LOAD', ondelete='cascade')
    shipper_load_status = fields.Text(
        string='Shipper Estado Odoo', compute="compute_odoo_load_status", store=True)
    provider_load_status = fields.Text(
        string='Proveedor Estado Odoo', compute="compute_odoo_load_status", store=True)

    @api.depends(
        'provider_account_line_load_id.account_move_id.payment_state',
        'shipper_account_line_load_id.account_move_id.payment_state'
    )
    ...
    def update_odoo_status(self):
        records = http.request.env['exo.odoo.conciliation'].sudo().search([
            '|',
            ('shipper_account_line_load_id', '=', False),
            ('provider_account_line_load_id', '=', False)
        ])
        index = 0
        for record in records:
            index += 1
            move_lines = http.request.env['account.line.load'].sudo().search([
                ('load_id', 'ilike', record.load_id)
            ])
            if move_lines:
                shipper_line_id = first([
                    move_line['id'] for move_line in move_lines if move_line['move_type'] == 'invoice'
                ])
                provider_line_id = first([
                    move_line['id'] for move_line in move_lines if move_line['move_type'] == 'bill'
                ])
                if (record.shipper_account_line_load_id != shipper_line_id or
                        record.provider_account_line_load_id != provider_line_id):
                    record.sudo().write({
    ...
    def start_conciliation(self):
        ...
        while True:
            skip += take
            url = (
                str(os.getenv('HOST_EXO')) +
                '/exo/loads/filterConsolidated/?skip=' +
                str(skip) + '&take=' + str(take) +
                '&useTimeSlot=true&query={}'
            )
            headers = {
            ...
            if response.status_code != 200:
                raise ValidationError(
                    'La carga no pudo ser procesada. 1. \nCode: %s\nContent: %s' %
                    (response.status_code, response.content)
                )
            result = response.json()
            if (not isinstance(result, collections.abc.Sequence) and
                    result["loads"] and not result["loads"]["Success"]):
                _logger.error(
                    'La carga no pudo ser procesada. 2. \nCode: %s\nContent: %s' %
                    (response.status_code, response.content)
                )
                raise ValidationError(
                    "La carga no pudo ser procesada.3. Favor comuniquese con su administrador e intente mas tarde"
                )
    ...
    def create_many_records(self, load_batched):
        load_records = [{
            ...
            'companyTransporter': load['companyTransporter'],
            'createdAt': load["loadingStatus"]["slotStartTime"],
            'load_status': load['load_status']
        } for load in load_batched]
    ...
    def get_odoo_loads_not_in_exo(self):
        ...
        try:
            ...
            load_records = [{
                'load_id': line.get('load_id'),
                'load_number': line.get('load_number'),
                'shipper': line.get('shipper'),
                'companyTransporter': None,
                'createdAt': None,
                'load_status': None
            } for line in data]
            http.request.env['exo.odoo.conciliation'].sudo().create(load_records)
