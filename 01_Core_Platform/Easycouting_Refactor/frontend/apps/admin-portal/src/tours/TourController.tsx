...
import { ADMIN_TOURS, type ViewTourDefinition } from "./catalog";
...
function resolveDefinition(pathname: string): ViewTourDefinition | null {
  return ADMIN_TOURS.find((tour) => tour.patterns.some((pattern) => patternToRegex(pattern).test(pathname))) ?? null;
}
