...
import { SELLER_TOURS, type ViewTourDefinition } from "./catalog";

function patternToRegex(pattern: string): RegExp {
  return new RegExp(`^${pattern.replace(/:[^/]+/g, "[^/]+")}$`);
}

function resolveDefinition(pathname: string): ViewTourDefinition | null {
  return SELLER_TOURS.find((tour) => tour.patterns.some((pattern) => patternToRegex(pattern).test(pathname))) ?? null;
}
