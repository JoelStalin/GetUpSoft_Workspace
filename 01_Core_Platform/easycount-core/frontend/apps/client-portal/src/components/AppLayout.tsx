...
export function AppLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      <aside className="hidden w-64 border-r border-border bg-surface/95 backdrop-blur-xl p-6 md:block">
        <div className="mb-10 space-y-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-[9px] uppercase tracking-[0.3em] text-primary/60 font-bold">Client Portal</p>
            </div>
          </div>
          ...
          {NAV.filter((item) => item.permissions.length === 0 || item.permissions.some((perm) => permissions.includes(perm))).map(
            (item) => (
              <NavLink
                ...
                to={item.to}
                className={({ isActive }) =>
                  `block rounded-md px-3 py-2 transition ${isActive ? "bg-primary/10 text-primary" : "text-muted hover:text-foreground hover:bg-white/5"}`
                }
              >
                {item.label}
              </NavLink>
      ...
      <main className="flex-1">
        <header className="flex items-center justify-between border-b border-border bg-background/60 backdrop-blur px-6 py-4">
          <div data-tour="session-user">
            <p className="text-[10px] uppercase tracking-[0.2em] text-muted">Portal sesión</p>
