    ...
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-300">Define reglas de monetizaciÃ³n para los tenants de getupsoft.</p>
        </div>
      ...
      <DataTable
        ...
        columns={[
          ...
          { header: "MÃ¡x/mes", cell: (row) => <span className="text-sm text-slate-300">{row.max_facturas_mes || "â€”"}</span> },
          {
            header: "MÃ¡x/cliente",
            cell: (row) => <span className="text-sm text-slate-300">{row.max_facturas_por_receptor_mes || "â€”"}</span>,
          },
          {
            header: "MÃ¡x monto",
            cell: (row) => (
