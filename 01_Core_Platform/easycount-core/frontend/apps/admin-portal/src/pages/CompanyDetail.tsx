    ...
    <div className="space-y-6">
      <header className="space-y-1">
        <h1 className="text-2xl font-semibold text-white">
          {tenantQuery.isLoading ? "Cargando compaÃ±Ã­aâ€¦" : tenant ? `CompaÃ±Ã­a: ${tenant.name}` : `Detalle de compaÃ±Ã­a #${id}`}
        </h1>
      ...
      {tenantQuery.isError ? (
        <div className="rounded-xl border border-rose-900/60 bg-rose-950/30 p-4 text-sm text-rose-200">
          No se pudo cargar el detalle de la compaÃ±Ã­a.
        </div>
      ) : null}
...
export function CompanyOverviewTab() {
  ...
  return (
    <div className="space-y-4 text-sm text-slate-300">
      <p>
        RNC emisor: <span className="font-mono text-slate-100">{tenantQuery.isLoading ? "â€¦" : tenant?.rnc ?? "â€”"}</span>
      </p>
      <p>
        Ambiente actual: <span className="font-semibold text-primary">{tenantQuery.isLoading ? "â€¦" : tenant?.env ?? "â€”"}</span>
      </p>
      <p>Ãšltima sincronizaciÃ³n DGII: hace 12 minutos.</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
          ...
          <tbody>
            {invoiceItems.map((item) => (
              <tr key={item.id} className="border-b border-slate-800/80 hover:bg-slate-900/40">
                ...
                <td className="px-3 py-2 font-mono text-xs text-slate-400">{item.track_id ?? "â€”"}</td>
      ...
      <div className="flex items-center justify-between text-xs text-slate-400">
        <span>
          PÃ¡gina {invoices?.page ?? page} Â· Total {invoices?.total ?? 0}
        </span>
      ...
      <section className="space-y-3">
        ...
        <form onSubmit={handleSubmit} className="grid gap-3 rounded-xl border border-slate-800 bg-slate-900/40 p-4 md:grid-cols-2">
          ...
          <div className="col-span-full flex justify-end">
            ...
            >
              {createLedgerEntry.isPending ? "Guardandoâ€¦" : "Registrar asiento"}
            </button>
          </div>
        </form>
      </section>
          ...
          <table className="min-w-full divide-y divide-slate-800 text-sm text-slate-300">
            <thead className="bg-slate-900/60 text-xs uppercase tracking-wide text-slate-400">
              <tr>
                ...
                <th className="px-3 py-2 text-right">DÃ©bito</th>
                <th className="px-3 py-2 text-right">CrÃ©dito</th>
              </tr>
            </thead>
            <tbody>
              {ledger?.items.map((item) => (
                <tr key={item.id} className="border-b border-slate-800/80 hover:bg-slate-900/40">
                  ...
                  <td className="px-3 py-2 font-mono text-xs text-slate-400">{item.encf ?? "â€”"}</td>
              ...
              {ledger?.items.length === 0 && (
                <tr>
                  <td className="px-3 py-6 text-center text-sm text-slate-500" colSpan={6}>
                    AÃºn no se registran asientos contables para esta empresa.
                  </td>
                </tr>
              )}
              {!ledger && (
...
export function CompanyPlansTab() {
  ...
  return (
    <div className="space-y-4 text-sm text-slate-300">
      <p>
        Plan vigente: <span className="font-semibold text-primary">{tenantPlanQuery.isLoading ? "Cargandoâ€¦" : currentPlanName}</span>
      </p>

      {plansQuery.isError || tenantPlanQuery.isError ? (
        <div className="rounded-xl border border-rose-900/60 bg-rose-950/30 p-4 text-sm text-rose-200">
          No se pudo cargar la informaciÃ³n de planes.
        </div>
      ) : null}
      ...
      <div className="grid gap-3 rounded-xl border border-slate-800 bg-slate-900/40 p-4 md:grid-cols-3">
        ...
        <div className="flex items-end justify-end">
          <button
            ...
            type="button"
          >
            {assignPlan.isPending ? "Guardandoâ€¦" : "Guardar"}
          </button>
        </div>
      </div>
    </div>
...
export function CompanyCertificatesTab() {
  return (
    <div className="space-y-4 text-sm text-slate-300">
      <p>Ãšltimo certificado subido: 2024-05-01.</p>
...
export function CompanyUsersTab() {
  return (
    <div className="space-y-4 text-sm text-slate-300">
      <p>Usuarios delegados (RBAC multi-tenant) se mostrarÃ¡n aquÃ­.</p>
