  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-white">CompaÃ±Ã­as</h1>
      ...
      {tenantsQuery.isError ? (
        <div className="rounded-xl border border-rose-900/60 bg-rose-950/30 p-4 text-sm text-rose-200">
          No se pudo cargar el listado de compaÃ±Ã­as.
        </div>
      ) : null}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-lg space-y-4 rounded-xl border border-slate-800 bg-slate-950 p-6">
            <header className="space-y-1">
              <h2 className="text-lg font-semibold text-white">Crear compaÃ±Ã­a</h2>
            ...
            {createTenant.isError ? (
              <div className="rounded-lg border border-rose-900/60 bg-rose-950/30 p-3 text-sm text-rose-200">
                No se pudo crear la compaÃ±Ã­a. Verifica RNC y vuelve a intentar.
              </div>
            ) : null}
            ...
            <footer className="flex justify-end gap-2">
              ...
              <button
                ...
                type="button"
              >
                {createTenant.isPending ? "Creandoâ€¦" : "Crear"}
              </button>
            </footer>
          </div>
        </div>
