...
class DgiiReport(models.Model):
    ...
    _period_company_unique = models.Constraint(
    ...
    def _compute_607_fields(self):
        for rec in self:
            ...
            for inv in sale_line_ids:
                data["sale_records"] += 1
                data["sale_invoiced_amount"] += inv.invoiced_amount_original
            ...
            rec.sale_invoiced_amount = data["sale_invoiced_amount"] #abs(data["sale_invoiced_amount"])
    ...
    def get_point_sale_payment_dict(self):

        payment_method_info={}

        try:
            records = self.env["pos.payment.method"].search([])
            for payment_m in records:
                payment_method_info[payment_m.name] = payment_m.journal_id.l10n_do_payment_form
        except:
            #Solo en caso de que el modulo de POS no esta instalado
            pass
    ...
    def _get_sale_payments_forms(self, invoice_id,payment_method_dict):
        payments_dict = self._get_payments_dict()
        Payment = self.env["account.payment"]
        AccountMove = self.env["account.move"]
        l10n_do_payment_form = False
      
        if invoice_id.move_type == "out_invoice":
          
           
            for payment in invoice_id._get_invoice_payment_widget():
                move_id = AccountMove.browse(payment["move_id"])
                if move_id:
                    
                   

                    if payment["account_payment_id"]:
                       l10n_do_payment_form =  move_id.journal_id.l10n_do_payment_form
                    elif(payment.has_key('pos_payment_name') and  payment["pos_payment_name"] ):
                        l10n_do_payment_form = payment_method_dict[payment["pos_payment_name"]]
                    else:
                        l10n_do_payment_form = "swap"

                    
                    key = l10n_do_payment_form
                    if key:
                        if self.include_payment(invoice_id, move_id):
    ...
    def _get_607_operations_dict(self):
        return {
            "fiscal": {
                ...
                "qty": 0,
                "amount": 0,
                "name": "COMPROBANTE VÃLIDO PARA CRÃ‰DITO FISCAL",
            ...
            "debit_note": {
                ...
                "qty": 0,
                "amount": 0,
                "name": "COMPROBANTES NOTA DE DÃ‰BITO",
                "dgii_report_id": self.id,
            },
            "credit_note": {
                "sequence": 5,
                "qty": 0,
                "amount": 0,
                "name": "COMPROBANTES NOTA DE CRÃ‰DITO",
                "dgii_report_id": self.id,
            },
            "unique": {
                "sequence": 6,
                "qty": 0,
                "amount": 0,
                "name": "COMPROBANTE REGISTRO ÃšNICO DE INGRESOS",
                "dgii_report_id": self.id,
            },
            "special": {
                "sequence": 8,
    ...
    def _compute_607_data(self):
        for rec in self:
            ...
            payment_method_dict =  self.get_point_sale_payment_dict()
            ...
            for inv in invoice_ids:
                ...
                show_payment_date = self._include_in_current_report(inv)
                payments = self._get_sale_payments_forms(inv,payment_method_dict)
                values = {
                    "dgii_report_id": rec.id,
                    "line": line,
                    "rnc_cedula": rnc_ced[0] if rnc_ced else False,
                    ...
                    'invoiced_amount_original': inv.amount_untaxed_signed,
                    "invoiced_itbis": inv.get_invoiced_itbis(),
    ...
    def generate_report(self):
        if self.state == "generated":
            action = self.env.ref(
                "getupsoft_l10n_do_accounting_report.dgii_report_regenerate_wizard_action"
            ).read()[0]
    ...
    def get_606_tree_view(self):
        return {
            ...
            "view_id": self.env.ref(
                "getupsoft_l10n_do_accounting_report.dgii_report_purchase_line_tree"
            ).id,
    ...
    def get_607_tree_view(self):
        return {
            ...
            "type": "ir.actions.act_window",
            "view_id": self.env.ref(
                "getupsoft_l10n_do_accounting_report.dgii_report_sale_line_tree"
            ).id,
            "domain": [("dgii_report_id", "=", self.id)],
        }
    ...
    def get_608_tree_view(self):
        return {
            ...
            "type": "ir.actions.act_window",
            "view_id": self.env.ref(
                "getupsoft_l10n_do_accounting_report.dgii_cancel_report_line_tree"
            ).id,
            "domain": [("dgii_report_id", "=", self.id)],
        }
    ...
    def get_609_tree_view(self):
        return {
            ...
            "type": "ir.actions.act_window",
            "view_id": self.env.ref(
                "getupsoft_l10n_do_accounting_report.dgii_exterior_report_line_tree"
            ).id,
            "domain": [("dgii_report_id", "=", self.id)],
        }
...
class DgiiReportSaleLine(models.Model):
    ...
    invoiced_amount = fields.Float()
    invoiced_amount_original = fields.Float()
    invoiced_itbis = fields.Float()
