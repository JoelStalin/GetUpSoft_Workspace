...
class EasyCountFiscalService:
    ...
    def notify_invoice_created(self, move_id: int, encf: str, amount: float) -> dict:
        ...
        if not self.base_url or not self.api_key:
            return {'status': 'placeholder', 'notified': False, 'reason': 'not configured'}
        ...
        try:
            ...
            return {'status': 'placeholder', 'notified': False, 'reason': 'endpoint not implemented'}
    ...
    def sync_to_odoo_accounting(self, move_id: int) -> dict:
        ...
        Note: Currently a no-op placeholder. Will be wired to OdooAccountingSyncService.
        TODO: wire to OdooAccountingSyncService.sync_operation(move_id)
        """
        if not self.base_url or not self.api_key:
            return {'status': 'placeholder', 'synced': False, 'reason': 'not configured'}
        ...
        try:
            ...
            # TODO: uncomment when OdooAccountingSyncService endpoint is confirmed ready
            # response = requests.post(
            ...
            return {'status': 'placeholder', 'synced': False, 'reason': 'endpoint not implemented'}
        except Exception as e:
