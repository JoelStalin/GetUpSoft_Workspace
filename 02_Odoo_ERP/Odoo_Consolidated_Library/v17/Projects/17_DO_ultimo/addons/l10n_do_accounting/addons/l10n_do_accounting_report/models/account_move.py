...
class AccountMove(models.Model):
    ...
    @api.depends("state", "move_type", "payment_state", "payment_idss")
    ...
    def _get_payment_string(self):
        ...
        for payment in self._get_invoice_payment_widget():
            payment_ids = self.env["account.payment"].browse(
                payment.get("account_payment_ids")
            )
            move_id = False
            if payment_ids:
                if payment_ids.journal_id.type in ["cash", "bank"]:
                    p_string = payment_ids.journal_id.l10n_do_payment_form
            if not payment_ids:
            ...
            payment = p_string if payment_ids or move_id else "credit_note"
    ...
    def _get_payment_move_iterator(self, payment, inv_type, witheld_type):
        ...
        if payment_ids:
            if inv_type == "out_invoice":
                return [
                    move_line.debit
                    for move_line in payment_ids.move_line_ids
