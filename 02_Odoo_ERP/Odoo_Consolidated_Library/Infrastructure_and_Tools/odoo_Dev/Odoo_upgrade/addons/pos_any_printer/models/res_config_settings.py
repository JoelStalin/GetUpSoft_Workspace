...
class ResConfigSettings(models.TransientModel):
    ...
    pos_any_printer_ip = fields.Char(
        string='Any Printer IP Address',
        help="Local IP address of the Any Printer",
        compute='_compute_pos_any_printer_ip', store=True, readonly=False
    )
    ...
    def _compute_pos_any_printer_ip(self):
        for res_config in self:
            if not res_config.pos_other_devices:
                res_config.pos_any_printer_ip = ""
            else:
                res_config.pos_any_printer_ip = (
