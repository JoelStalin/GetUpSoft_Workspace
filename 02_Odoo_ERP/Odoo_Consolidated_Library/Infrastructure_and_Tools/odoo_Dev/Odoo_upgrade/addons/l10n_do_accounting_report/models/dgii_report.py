...
def _compute_previous_report_pending(self):
    for report in self:
        ...
        if previous:
            ...
            current_date = dt.strptime("01/" + report.name, "%d/%m/%Y").date()
            report.previous_report_pending = previous_date < current_date
        else:
...
def _compute_606_fields(self):
    for rec in self:
        ...
        data = {
            "purchase_records": len(purchase_line_ids),
            "service_total_amount": sum(purchase_line_ids.mapped('service_total_amount')),
            "good_total_amount": sum(purchase_line_ids.mapped('good_total_amount')),
            "purchase_invoiced_amount": sum(purchase_line_ids.mapped('invoiced_amount')),
            "purchase_invoiced_itbis": sum(purchase_line_ids.mapped('invoiced_itbis')),
            "purchase_withholded_itbis": sum(purchase_line_ids.mapped('withholded_itbis')),
            "cost_itbis": sum(purchase_line_ids.mapped('cost_itbis')),
            "advance_itbis": sum(purchase_line_ids.mapped('advance_itbis')),
            "income_withholding": sum(purchase_line_ids.mapped('income_withholding')),
            "purchase_selective_tax": sum(purchase_line_ids.mapped('selective_tax')),
            "purchase_other_taxes": sum(purchase_line_ids.mapped('other_taxes')),
            "purchase_legal_tip": sum(purchase_line_ids.mapped('legal_tip')),
        }
        for field, value in data.items():
            setattr(rec, field, abs(value))
...
def _compute_607_fields(self):
    for rec in self:
        ...
        data = {
            "sale_records": len(sale_line_ids),
            "sale_invoiced_amount": sum(sale_line_ids.mapped('invoiced_amount')),
            "sale_invoiced_itbis": sum(sale_line_ids.mapped('invoiced_itbis')),
            "sale_withholded_itbis": sum(sale_line_ids.mapped('third_withheld_itbis')),
            "sale_withholded_isr": sum(sale_line_ids.mapped('third_income_withholding')),
            "sale_selective_tax": sum(sale_line_ids.mapped('selective_tax')),
            "sale_other_taxes": sum(sale_line_ids.mapped('other_taxes')),
            "sale_legal_tip": sum(sale_line_ids.mapped('legal_tip')),
        }
        for field, value in data.items():
            setattr(rec, field, abs(value))
...
def _generate_606_txt(self, records, qty):
    ...
    header = f"606|{company_vat}|{period}|{qty}\n"
    data = header + records

    file_path = f"{tempfile.gettempdir()}/DGII_606_{company_vat}_{period}_{self.id}.txt"
