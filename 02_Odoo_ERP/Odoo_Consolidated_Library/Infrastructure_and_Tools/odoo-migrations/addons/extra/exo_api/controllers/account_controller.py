...
class AccountController(http.Controller):
    ...
    def create(self):
        # FIX: Reintentos para manejar SerializationFailure en concurrencia masiva
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt == retries - 1:
                    _logger.error("SerializationFailure persistente en /api/account/create/")
                    return http_handler_error("Concurrency error. Please try again.", 500, '500 Internal Server Error')
                _logger.warning(f"Reintentando creación por concurrencia (Intento {attempt + 1}/{retries})")
                continue
            except Exception as e:
                return http_handler_error(str(e), 500, '500 Internal Server Error')
