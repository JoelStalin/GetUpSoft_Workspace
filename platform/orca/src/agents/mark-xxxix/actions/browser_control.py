...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".Orca_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".Orca_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using Orca profile")
                Orca = str(Path.home() / ".Orca_profiles" / "firefox_Orca")
                Path(Orca).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(Orca, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".Orca_profiles" / "safari")
        ...
        Orca_profile = str(Path.home() / ".Orca_profiles" / self.browser_name)
        Path(Orca_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with Orca profile: {Orca_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(Orca_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] âœ… Launched [{label}] with Orca profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "Orca_screenshot.png")
