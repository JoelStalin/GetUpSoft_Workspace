...
* [Actions](#actions)
  ...
  * [AI Assisted Replies](#ai-assisted-replies)
...
# Actions
## AI Assisted Replies
`TinderBotJ` now includes an assisted reply workflow for human-reviewed drafting.

It is designed to:

- extract recent visible chat context;
- build a conversation context from profile + recent messages;
- route the request through a selected model strategy;
- estimate provider token cost;
- add a 50% service fee;
- record the event to `data/ai_usage/usage_log.jsonl`;
- require explicit human approval before any message is sent.

It is **not** designed for autonomous romantic chatting, manipulative persuasion, or automatic date-closing flows.
...
Configure the API key that matches the strategy you use:
...
In `quickstart.py`, choose:

```text
3. Modo Asistido IA (Sugerencias con aprobación humana)
```
...
Only if you select a numbered option will the message be sent.
