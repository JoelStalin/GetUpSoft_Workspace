...
* Suggesting AI-assisted replies with human approval before send
...
### Assisted AI reply mode
`TinderBotJ` now includes an assisted mode that:

- extracts recent visible chat context;
- builds reply suggestions for human review;
- supports multiple model strategies (`cheap`, `balanced`, `premium`);
- estimates provider token cost and adds a 50% service fee;
- logs each assisted generation to `data/ai_usage/usage_log.jsonl`;
- only sends a message after explicit user approval.

This mode is intentionally **assistive only**. It does not auto-chat, does not impersonate a human autonomously, and does not attempt manipulative conversation flows.

### Reference material used for the assisted mode
Conversation playbooks and guardrails were oriented around ethical communication references such as:

- *Crucial Conversations*
- *Nonviolent Communication*
- *Difficult Conversations*

These references were used to shape tone, empathy, curiosity, and consent-focused prompts rather than manipulative scripts.

### Similar public repositories used as starting references

- `frederikme/TinderBotz`
- `joeVenner/AI-Tinder-BOT`
- `TheoMarkopoulos/agent-router`
- `zackham/aloop`
...
The new AI-assisted reply flow is designed for **human-in-the-loop drafting** only. You are responsible for reviewing any generated message before sending it.
