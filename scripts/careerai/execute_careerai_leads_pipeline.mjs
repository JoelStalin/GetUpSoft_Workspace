...
for (let i = 0; i < RAW_LEADS.length; i++) {
  ...
  const trackingItem = {
    ...
    application_status: 'SUBMITTED_AND_TRACKED',
...
const csvHeaders = ['ID', 'Platform', 'Company', 'Title', 'Language', 'Method', 'Contact_Email', 'Job_URL', 'Status', 'Submitted_At', 'Followup_1', 'Followup_2', 'CV_File', 'Letter_File'];
...
for (const item of processedTracking) {
  csvRows.push([
    ...
    item.submission_timestamp.split('T')[0],
